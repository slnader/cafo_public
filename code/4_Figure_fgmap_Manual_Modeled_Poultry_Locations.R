...
#################################################
  ...
  all.cafos <- cafo.locations[,c("facility_id", "latitude", "longitude", "true_positive", "new_cafo")]
  all.cafos$animal <- "POULTRY"
  all.cafos$source <- "model"
  all.cafos <- all.cafos[,c("facility_id", "latitude", "longitude", "animal", "true_positive", "source", "new_cafo")]
  ...
  cols <- paste(c("all_positive", "true_positive", "new_cafo"), "POULTRY", sep =".")
...
#################################################
  ...
  pdf("../figures/4_Figure_fgmap_Manual_Modeled_Poultry_Locations.pdf", width = 8.5, height = 8)
    layout(matrix(c(1,1,1,3,1,1,2,2,2,4,2,2),4,3,byrow=T),width=c(0.4,0.5,0.5), height=c(0.4,0.16,0.4,0.16))
    ...
    for(this.source in c("model", "new")){
      ...
      #Heatmap column
      heatmap.column <- ifelse(this.source%in%c("model"), "all_positive.POULTRY", "new_cafo.POULTRY")
      ...
      #Determine title
      this.title <- ifelse(this.source=="model", "Modelled locations", "New locations")
