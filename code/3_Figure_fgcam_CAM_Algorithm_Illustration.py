...
def generate_poly(cam_array, threshold):
    ...
    threshold = value between 0 and 1 to define bounding box as percentage
    of max cam value
...
def get_pixel_clusters(poly, distance_threshold=150, start_clusters=5):
    """
    Function to obtain ordered array of polygon outline for visualization
    purposes

    Keyword Arguments:
    poly = unordered array of pixel location tuples (x,y)
    distance_threshold = distance in meters that we require clusters to be
    separated by
    ...
    for c in possible_clusters:
        separated_clusters = np.argwhere((center_distances[c]> 0) & \
        (center_distances[c] < distance_threshold))
    ...
    kmeans = KMeans(n_clusters=len(clusters_to_keep),
    random_state=94063).fit(poly)
...
def run_cam_loop(image_path,  sess, start_clusters = 5):
    ...
    #Get final weights
    weight_softmax = sess.graph.get_tensor_by_name(
    'import/final_training_ops/weights/final_weights:0').eval()
    ...
    pixel_clusters, pixel_labels = get_pixel_clusters(activated_pixels, 150,
    start_clusters)
...
def main():
    ...
    #PIL fonts
    bold_font = ImageFont.truetype(os.path.join(data_dir,
    'fonts/DejaVuSans-Bold.ttf'), 16)
    font = ImageFont.truetype(os.path.join(data_dir,
    'fonts/DejaVuSans.ttf'), 16)
    ...
    square_footage_df['image_cluster_id'] = [str(square_footage_df['image_id'][i])
    ...
    with tf.Session(graph=poultry_graph) as sess:
        ...
        #Get cam results
        activated_pixels, pixel_clusters, pixel_labels,
        final_result = run_cam_loop(img_path, sess)
    ...
    #Original image
    org_im = PILImage.open(os.path.join(image_dir, image_id + \
    '.jpeg')).convert('RGB')
    ...
    draw_image.text((120,10), 'Poultry score: ' + \
    '{0:1.2f}'.format(poultry_score), 'white', font=bold_font)
    draw_image.text((130,30), 'Swine score: ' + \
    '{0:1.2f}'.format(swine_score), 'white', font=bold_font)
    new_im.paste(org_act, (299,0))

    #Centered image
    cent_im = PILImage.open(os.path.join(image_dir, image_id + \
    '_0.jpeg')).convert('RGB')
    ...
    draw_image.text((120,10), 'Poultry score: ' + \
    '{0:1.2f}'.format(new_poultry_score), 'white', font=bold_font)
    draw_image.text((130,30), 'Swine score: ' + \
    '{0:1.2f}'.format(new_swine_score), 'white', font=bold_font)
    new_im.paste(cent_act, (299,299))

    #Save image
    new_im.save(os.path.join(fig_dir,
    '3_Figure_fgcam_CAM_Algorithm_Illustration.png'), dpi=(300,300))
