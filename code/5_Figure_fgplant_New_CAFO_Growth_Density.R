...
#################################################
  #Model results
  facility.data <- read.csv("../data/csv/longitudinal_image_data.csv",
  stringsAsFactors = F)

  #Point type
  facility.data$point.type <- "Existing CAFO"
  facility.data$point.type[which(facility.data$true_naip_year>=2011 &
    facility.data$image_id!="Poultry Plant")] <- "New CAFO"
  facility.data$point.type[which(
    facility.data$image_id=="Poultry Plant")] <- "Plant Location"
...
#################################################
  ...
  city_map <- get_map(location = c(lon = plant.coords[1],
    lat =  plant.coords[2]), zoom = 9, maptype = "hybrid", api_key = api.key)
  ...
  this.map <- ggmap(city_map) +
    stat_density2d( data = master.map.data[which(
      master.map.data$point.type=="New CAFO"),],
                    geom="polygon",
                  aes( x = longitude, y = latitude, fill = ..level.. ,
                    alpha = ..level..) ) +
    geom_point(x=plant.coords[1], y = plant.coords[2], col="white")+
    geom_text(x=plant.coords[1], y = plant.coords[2]-0.05,
      label = "Feed mill", size = 1.5, col="white",
              family = 'DejaVu')+
