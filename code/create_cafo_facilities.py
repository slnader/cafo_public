...
def main():
    ...
    new_score_df.loc[(((new_score_df.poultry_score >= score_threshold) | \
    (new_score_df.swine_score >= score_threshold)) & \
    (new_score_df.poultry_score > new_score_df.swine_score)),
    'new_score_class'] = 'poultry cafo'
    new_score_df.loc[(((new_score_df.poultry_score >= score_threshold) | \
    (new_score_df.swine_score >= score_threshold)) & \
    (new_score_df.poultry_score <= new_score_df.swine_score)),
    'new_score_class'] = 'swine cafo'

    #Add image cluster id
    square_footage_df['image_cluster_id'] = [
    str(square_footage_df['image_id'][i]) + \
    '_'+ str(square_footage_df['cluster'][i]) + '.jpeg'
    for i in range(len(square_footage_df))]

    #Convert strings to arrays
    square_footage_df.activation_bounds = [x.replace("[", "").replace("]", "").\
    replace(" ", "") for x in square_footage_df.activation_bounds]
    square_footage_df.activation_bounds = [x.split(",")
    for x in square_footage_df.activation_bounds.values]
    ...
    #Add county name
    square_footage_df["county_name"] = [x.split('_')[1]
    for x in square_footage_df.image_id]

    #Merge to new classification
    square_footage_df = square_footage_df.merge(new_score_df[
    ['image_cluster_id', 'new_score_class', 'poultry_score']],
    on = 'image_cluster_id', how = 'left')

    #Add boundary image flag (does the object touch the edge of the image)
    square_footage_df['min_bounds'] = [
    np.min(x) for x in square_footage_df.activation_bounds.values]
    square_footage_df['max_bounds'] = [
    np.max(x) for x in square_footage_df.activation_bounds.values]
    square_footage_df['boundary_image'] = 0
    square_footage_df.loc[((square_footage_df.min_bounds==0) | \
    (square_footage_df.max_bounds>=298)), 'boundary_image'] = 1
    ...
    for this_zone in list(set(square_footage_df.zone.values)):
        ...
        #Get data for zone
        this_zone_data = square_footage_df.loc[(
        square_footage_df.zone==this_zone), ]
        min_distances = [0]
        j = 0

        while np.min(min_distances) <= distance_threshold:

            print("""Round %d for utm zone %d: minimum distance between
            centroids is %f."""%(j, this_zone, np.min(min_distances)))
            #Reset index and calculate distances
            this_zone_data = this_zone_data.reset_index(drop=True)
            this_zone_distances = euclidean_distances(this_zone_data[['easting',
            'northing']])

            #Get min distance
            distances_no_diagonal = [np.delete(this_zone_distances[i],i)
            for i in range(len(this_zone_distances))]
            min_distances = [np.min(x) for x in distances_no_diagonal]

            #Initialize results
            result_df = pd.DataFrame(columns = list(square_footage_df) + \
            ['is_poultry'])
            reviewed_indices = []
            ...
            for i in range(len(this_zone_distances)):
                ...
                if min_distances[i] <= distance_threshold:
                    ...
                    pair_data = this_zone_data.iloc[(
                    this_zone_data.index.isin(cluster_indices)),]
                    pair_data = pair_data.sort_values(['poultry_score',
                    'boundary_image', 'square_footage'],
                    ...
                    if is_poultry:
                        average_easting = np.mean(pair_data.loc[(
                        pair_data.new_score_class=='poultry cafo'), 'easting'])
                        average_northing = np.mean(pair_data.loc[(
                        pair_data.new_score_class=='poultry cafo'), 'northing'])
                    else:
                    ...
                    square_footage_df.loc[(
                    square_footage_df.image_cluster_id.isin(cluster_images)),
                    'image_group'] = pair_data['image_id']
                    ...
                    #Update square footage df
                    square_footage_df.loc[(
                    square_footage_df.image_cluster_id.isin([
                    pair_data['image_cluster_id']])),
                    'image_group'] = pair_data['image_id']
    ...
    #Create facility id
    all_results_df['facility_id'] = all_results_df['image_id'] + '_' + \
    all_results_df['cluster'].map(str) + all_results_df['center'].map(str)
    ...
    all_results_df= all_results_df[['facility_id', 'latitude', 'longitude',
    'square_footage']]

    #Write out predicted positives to csv
    all_results_df.to_csv(os.path.join(data_dir, 'csv',
    'consolidated_facilities.csv'), index = False)
