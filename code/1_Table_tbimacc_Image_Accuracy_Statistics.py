...
def main():
    ...
    for this_model in [poultry_model_version, swine_model_version]:
        ...
        for this_threshold in [[0,1], [1]]:
            ...
            model_data_set = accuracy_df.loc[(
            (accuracy_df.model_version==this_model) & 
            ((accuracy_df.cafo_threshold.isin(this_threshold)) |
             (accuracy_df.image_class_cat=='notcafo'))),]

            #Area under ROC curve
            this_auc = skmetrics.roc_auc_score(
            model_data_set[['image_class_binary']],
            ...
            len(model_data_set.loc[(
            model_data_set.image_class_cat==model_classes[0]),]),
            len(model_data_set.loc[(
            model_data_set.image_class_cat==model_classes[1]),]))
            all_results.append(result_tuple)

    #Convert results to tabular
    tab_results = pd.DataFrame(all_results, columns = ['model', 'sample', 'auc',
    'avg_precision', 'n_1', 'n_0'])
    ...
    #Write to csv
    tab_results.to_csv(os.path.join(tab_dir,
    "1_Table_tbimacc_image_accuracy_statistics.csv"), index = False)
